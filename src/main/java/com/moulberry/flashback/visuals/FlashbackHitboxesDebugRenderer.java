...
/**
 * Renders entity hitboxes (bounding boxes) for all entities, including during export.
 * Unlike vanilla F3+B this draws clean boxes only: no eye-level lines, no view vectors.
 * Controlled by ReplayVisuals.renderHitboxes; checked per-frame so no renderer refresh
 * is needed when the checkbox is toggled.
 */
